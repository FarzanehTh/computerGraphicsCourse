...
{   
    ...
    if (is_moon) {
        ...
        vec3 t = vec3(2.0, 0, 0);
        pos_cs_in = translate(t) * uniform_scale(0.3) * vec4(pos_vs_in, 1.0);
    }

    // Animate with the frequency of 1 revolution per 4 seconds, then 
    // project to "camera" space and then get its projection
    pos_cs_in = proj * view * model(is_moon, animation_seconds) * pos_cs_in;
}
